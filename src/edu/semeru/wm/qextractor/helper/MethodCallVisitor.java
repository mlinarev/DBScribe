...
public class MethodCallVisitor extends ASTVisitor{
	...
	public boolean visit(MethodInvocation call){
			...
			if(parent instanceof MethodDeclaration){
				...
				if(args != null){
					...
					for (Object expr : args) {
						...
						try{
							callerArgs += temp2.getType().toString()+",";
		...
		// only for gesdes
		/*if(value.contains("DAO") && !value.contains("Impl")){
			value = value.replace("DAO|","DAOImpl|").replace("dao.","dao.impl.");
		}
		if(value.contains("Service") && !value.contains("Impl")){
			value = value.replace("Service|","ServiceImpl|").replace("services.","services.impl.");
		}
		
		if(key.contains("DAO") && !key.contains("Impl")){
			key = key.replace("DAO|","DAOImpl|").replace("dao.","dao.impl.");
		}
		if(key.contains("Service") && !key.contains("Impl")){
			key = key.replace("Service|","ServiceImpl|").replace("services.","services.impl.");
		}*/
